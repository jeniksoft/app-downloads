...
| Executor | MCP/plugin/app result lifecycle | `partial` | Existující read-only app connector nyní po skutečném PolicyExecutor runtime předá typed receipt do WorkerJournal před dispatch-done; deterministický self-test a source guard prošly. Obecný MCP/plugin transport a live acceptance zůstávají otevřené.; mezera: Dodat generic typed fixtures pro success, partial, failure, timeout, retry a owner review; MCP/plugin stdio, mutující extension třídy a owner-approved live acceptance nejsou uzavřené.; typy: documentation, self-test |
...
| `a1955784cbdf` | 2026-08-24 | evidence-only | — |
