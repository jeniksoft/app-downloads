...
| UX/support | Failure/recovery a ticket UI | `partial` | Failure/recovery a Secure Ticket kontrakty existují; P6.1 typovaný stavový automat, P6.2 read-only evidence candidate refs, P6.3 shared sensitive-data boundary, P6.4 protected local draft store, P6.5 owner evidence selection review, P6.6 immutable prepared manifest digest, P6.7 bounded local export package a P6.8 read-only untrusted reproduction intake mají build/self-test důkaz, ale P6 runtime, P6.9 a P8 QA zůstávají partial.; mezera: P6.7 local export a P6.8 untrusted import/reproduction jsou ověřené bez sítě; otevřené zůstávají P6.9 lineage receipt, kompletní ticket UI path, runtime a owner-facing V5 acceptance.; typy: contract, release-gate, self-test |
...
| `5484c6ec1f2f` | 2026-08-24 | evidence-only | — |
