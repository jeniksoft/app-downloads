...
| UX/support | Failure/recovery a ticket UI | `partial` | Failure/recovery a Secure Ticket kontrakty existují; P6.1 typovaný stavový automat, P6.2 read-only evidence candidate refs, P6.3 shared sensitive-data boundary, P6.4 protected local draft store a P6.5 owner evidence selection review mají build/self-test důkaz, ale immutable manifest, P6 runtime a P8 QA zůstávají partial.; mezera: Dodat immutable prepared manifest, lineage receipt a kompletní ticket UI path; owner-facing V5 acceptance zůstává otevřená.; typy: contract, release-gate, self-test |
...
| `b3ca44cf6683` | 2026-08-24 | evidence-only | — |
